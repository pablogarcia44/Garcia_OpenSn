...
mat_names = {'clad_gado', 'clad_guide', 'clad_instru', 'clad_pincell', 'fuel_gado', 'fuel_pincell', 'gap_gado', 'gap_pincell', 'moderator_gado', 'moderator_guide', 'moderator_instru', 'moderator_pincell', 'water_guide', 'water_instru', 'water_outside'}
materials = {}
my_xs = {}
xs_file = 'mgxs_2P.h5'
...
lbs_options = {
    ...
    write_restart_path = "2P_restart/2P",
}
...
for i = 1, num_cells do
    ...
    for j = 1, num_cells do
        ...
        if not should_avoid(i, j) then
            ...
            for _, pos in ipairs(d['gado']) do
                if pos[1] == (i - 1) and pos[2] == (j - 1) then
                    family_name = 'fuel_gado'
                    break
                end
            end
...
MPIBarrier()
log.Log(LOG_0,'ici')
fieldfunc.ExportToVTKMulti(fflist,'flux_2P')