...
my_filename = "lattice_17x17_1fam.obj"
...
mat_names = {'clad_guide', 'clad_instru', 'clad_pincell', 'fuel_pincell', 'gap_pincell', 'moderator_guide', 'moderator_instru', 'moderator_pincell', 'water_guide', 'water_instru', 'water_outside'}
materials = {}
my_xs = {}
xs_file = 'mgxs_1_10.h5'
...
for i = 1, num_cells do
  ...
  for j = 1, num_cells do
      ...
      if not should_avoid(i, j) then
          ...
          -- Suppose xs_fuel_pincell is already defined
          xs_fuel_pincell = xs.Get(my_xs["fuel_pincell"])
...
if location_id == 0 then
  ofile = io.open('pin_power_on.txt',"w")
  io.output(ofile)  
  for i = 1, num_cells do
    for j = 1, num_cells do 
...
MPIBarrier()
log.Log(LOG_0,'ici')
fieldfunc.ExportToVTKMulti(fflist,'pin_10fuel')
